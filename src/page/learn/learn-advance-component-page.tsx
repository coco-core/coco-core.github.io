  ...
  render() {
    return <ContentLayout sideMenu={<SideMenu />}>
      ...
      * @router
      ...
      一共三层，可以理解为: component用于添加一个无概念组件。第二层定义了具有通用概念的组件。第三层在通用概念的范围内的再划分的组件。
      ...
      * 第三方组件
      ...
      2. 2和3其实是一样的，通过配置当前组件的优先级最高的子组件id，getComponent在发现存在多个子组件的时候，从配置文件中确定优先级最高的子组件，然后进行实例化。

      ### 组件id
      id是字符串类型，一般来说是类名，第三方提供公共前缀的功能

      1. 提供接口返回所有子组件类型+运行时配置文件
      2. 通过装饰器确定（在定义子组件的时候就确定了）
